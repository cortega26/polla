...
- Minimal spans are emitted around major phases (e.g., `ingestion_orchestration`).
- Example:

```json
{"event":"span_start","name":"ingestion_orchestration","attrs":{"sources":["pozos"]},"timestamp":"...","correlation_id":"..."}
{"event":"span_end","name":"ingestion_orchestration","ms":1234,"timestamp":"...","correlation_id":"..."}
```
...
- Grafana + Loki (or any JSON log backend):
  ...
  - Panel 2: `quantile_over_time(0.95, {event="span_end", name="ingestion_orchestration"} | unwrap ms [24h])`.
...
- Example Loki LogQL (conceptual):
  ...
  - `histogram_quantile(0.95, sum by (le) (rate({event="span_end", name="ingestion_orchestration"} | unwrap ms | __error__="" [5m])))`
...
- Latency p95 exceeded: `quantile_over_time(0.95, {event="span_end", name="ingestion_orchestration"} | unwrap ms [15m]) > 15000`

## Verification Checklist

- [ ] Run `python -m polla_app run --log-file logs/run.jsonl` and confirm log lines contain `correlation_id`.
- [ ] Confirm span start/end appear with `name="ingestion_orchestration"` and `ms` value.
